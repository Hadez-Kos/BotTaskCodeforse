...
async def get_data_page(
    table_list: list,
):  # Осуществляем сбор данных по каждой задаче в таблице
    list_task = list()
    try:
...
async def add_data_database(data_json: list):
    themes = set()
    for i in data_json:
        await Database().insert_data_task(
            {
                "solution": i["solution"],
                "name": i["name"],
                "number": i["number"],
...
async def main_parse():
    # изменение url страницы для чтения всех страниц
    number_page = 1
    ...
    while True:
        url_site = (
            component_url[0] + "/page/" + str(number_page) + f"?{component_url[1]}"
        )
        ...
        try:
            if table_tasks:
                tasks += await get_data_page(table_tasks)
            else:
                break
        except:
            break
