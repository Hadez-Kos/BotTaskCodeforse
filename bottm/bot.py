...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        "\n2) Search - поиск задачи.", reply_markup=keyboard, )
...
async def cmd_search(message: types.Message):
    ...
    await message.answer(
        ...
        "\n4) Back - вернуться", reply_markup=keyboard, )
...
async def cmd_search(message: types.Message):
    ...
    await message.answer(
        "Оп, вернулись", reply_markup=keyboard, )
...
async def cmd_search(message: types.Message):
    ...
    for theme in await Database().get_data_theme():
        builder.row(types.InlineKeyboardButton(
            text=f"{theme}",
            callback_data=f"call_theme-{theme}"
        ))
...
async def cmd_solution(message: types.Message):
    ...
    for sol in await Database().get_data_solution(request['theme']):
        builder.row(types.InlineKeyboardButton(
            text=f"{sol}",
            callback_data=f"call_solution-{sol}"
        ))
...
async def main():
    ...
    threading.Timer(3600, asyncio.run(main_parse())).start()
